 ...
 */
function bswp_count_number_of_children($item_id) {
    global $wpdb;
    ...
    $sql = $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '%s' AND meta_value = '%s' LIMIT %d,%d", $meta_key, $item_id, $offset, $limit );
 ...
 * @uses Walker
 */
class BSWP_Walker_Nav_Menu extends Walker {
	/**
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        /* BSWP-specific: Bootstrap does not support 'deep' dropdown menus right now, maybe someday? */
        if ($depth >= 2) {
 ...
 */
function bswp_nav_menu( $args = array() ) {
	...
	$defaults = array( 
                      ...
                      'link_after' => '', 
                      'menu_before' => '',
                      'menu_after' => '<a href="#" class="bswp-navpair-right dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a>', 
	...
	bswp_menu_item_classes_by_context( $menu_items );
	...
	$items .= bswp_walk_nav_menu_tree( $sorted_menu_items, $args->depth, $args );
 ...
 */
function bswp_menu_item_classes_by_context( &$menu_items ) {
 ...
 */
function bswp_walk_nav_menu_tree( $items, $depth, $r ) {
	$walker = ( empty($r->walker) ) ? new BSWP_Walker_Nav_Menu : $r->walker;
 ...
 * @access private
 */
function bswp__nav_menu_item_id_use_once( $id, $item ) {
