 ...
 */
function bswp_the_title($before = '', $after = '', $echo = true) {
	$title = get_the_title();
 ...
 * an array. See the function bswp_for what can be override in the $args parameter.
 *
 ...
 */
function bswp_the_title_attribute( $args = '' ) {
 ...
 * @return string
 */
function bswp_get_the_title( $post = 0 ) {
	$post = get_post( $post );
 ...
 * @return string
 */
function bswp_get_the_guid( $id = 0 ) {
	$post = get_post($id);
 ...
 */
function bswp_the_content( $more_link_text = null, $strip_teaser = false) {
 ...
 * @return string
 */
function bswp_get_the_content( $more_link_text = null, $strip_teaser = false ) {
 ...
 * @return string
 */
function bswp__convert_urlencoded_to_entities( $match ) {
 ...
 * @return string
 */
function bswp_get_the_excerpt( $deprecated = '' ) {
	if ( !empty( $deprecated ) )
 ...
 * @return bool
 */
function bswp_has_excerpt( $id = 0 ) {
	$post = get_post( $id );
 ...
 */
function bswp_post_class( $class = '', $post_id = null ) {
 ...
 */
function bswp_get_post_class( $class = '', $post_id = null ) {
 ...
 * @return array Array of classes.
 */
function bswp_get_body_class( $class = '' ) {
 ...
 */
function bswp_post_password_required( $post = null ) {
	$post = get_post($post);
 ...
 */
function bswp_wp_link_pages( $args = '' ) {
	$defaults = array(
		'before'           => '<p>' . __( 'Pages:' ),
		'after'            => '</p>',
		'link_before'      => '',
...
/**
 * Helper function bswp_for wp_link_pages().
 *
 * @since 3.1.0
 * @access private
 *
 ...
 */
function bswp_wp_dropdown_pages($args = '') {
	$defaults = array(
		'depth' => 0, 'child_of' => 0,
		'selected' => 0, 'echo' => 1,
		'name' => 'page_id', 'id' => '',
 ...
 * @return string HTML content, if not displaying.
 */
function bswp_wp_list_pages($args = '') {
	$defaults = array(
		'depth' => 0, 'show_date' => '',
 ...
 * wp_list_pages()} function. Check that function bswp_for more info on those
 * arguments.
 *
 * <ul>
 ...
 */
function bswp_page_menu( $args = array() ) {
 ...
 */
function bswp_walk_page_tree($pages, $depth, $current_page, $r) {
	if ( empty($r['walker']) )
		$walker = new BSWP_Walker_Page;
	else
		$walker = $r['walker'];
 ...
 */
function bswp_walk_page_dropdown_tree() {
	$args = func_get_args();
	if ( empty($args[2]['walker']) ) // the user's options are the third parameter
		$walker = new BSWP_Walker_PageDropdown;
	else
 ...
 * @uses Walker
 */
class BSWP_Walker_Page extends Walker {
	/**
	 ...
	 * @param array $args
	 */
	function bswp_start_lvl( &$output, $depth = 0, $args = array() ) {
	 ...
	 * @param array $args
	 */
	function bswp_end_lvl( &$output, $depth = 0, $args = array() ) {
		$indent = str_repeat("\t", $depth);
		$output .= "$indent</ul>\n";
	}
	 ...
	 * @param array $args
	 */
	function bswp_start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		if ( $depth )
			$indent = str_repeat("\t", $depth);
		else
			$indent = '';
	 ...
	 * @param array $args
	 */
	function bswp_end_el( &$output, $page, $depth = 0, $args = array() ) {
		$output .= "</li>\n";
	}
 ...
 * @uses Walker
 */
class BSWP_Walker_PageDropdown extends Walker {
	/**
	 * @see Walker::$tree_type
	 * @since 2.1.0
	 * @var string
	 ...
	 * @param int $id
	 */
	function bswp_start_el( &$output, $page, $depth = 0, $args = array(), $id = 0 ) {
 ...
 */
function bswp_the_attachment_link( $id = 0, $fullsize = false, $deprecated = false, $permalink = false ) {
	if ( !empty( $deprecated ) )
 ...
 */
function bswp_wp_get_attachment_link( $id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false ) {
	$id = intval( $id );
	$_post = get_post( $id );
 ...
 * @return string
 */
function bswp_prepend_attachment($content) {
	$post = get_post();
 ...
 */
function bswp_get_the_password_form( $post = 0 ) {
	$post = get_post( $post );
 ...
 */
function bswp_is_page_template( $template = '' ) {
	if ( ! is_page() )
		return false;
 ...
 */
function bswp_get_page_template_slug( $post_id = null ) {
	$post = get_post( $post_id );
 ...
 */
function bswp_wp_post_revision_title( $revision, $link = true ) {
 ...
 */
function bswp_wp_post_revision_title_expanded( $revision, $link = true ) {
	if ( !$revision = get_post( $revision ) )
		return $revision;
 ...
 * @return null
 */
function bswp_wp_list_post_revisions( $post_id = 0, $type = 'all' ) {
